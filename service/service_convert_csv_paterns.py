...
def processar_enderecos_otimizado(
    df_enderecos, df_roteiro_aparecida, df_roteiro_goiania
):
    """
    ...
    if "COMPLEMENTO3" in df.columns:
        ...
        df["COMPLEMENTO3_TRATADO"] = (
            df["COMPLEMENTO3"].fillna("").astype(str).str.strip().str.upper()
        )
    else:
    ...
    if len(df_com_prefixo) > 0:
        df_com_prefixo["ORDEM"] = (
            df_com_prefixo.groupby(["CHAVE_LOG", "Prefixo"]).cumcount() + 1
        )
        df_com_prefixo["Resultado"] = (
            df_com_prefixo["Prefixo"] + " " + df_com_prefixo["ORDEM"].astype(str)
        )
    ...
    df["COMPARATIVO"] = np.where(
        df["RESULTADO"] == df["COMPLEMENTO3_TRATADO"], "VERDADEIRO", "FALSO"
    )
    ...
    df_roteiros = pd.concat(
        [df_roteiro_aparecida, df_roteiro_goiania], ignore_index=True
    )

    # Prepara colunas para merge - CORREÇÃO DO ERRO
    if "id" in df_roteiros.columns:
        df_roteiros["id"] = (
            df_roteiros["id"].astype(str).str.replace(r"\.0$", "", regex=True)
        )
    if "id_localidade" in df_roteiros.columns:
        df_roteiros["id_localidade"] = (
            df_roteiros["id_localidade"]
            .astype(str)
    ...
    if "cod_lograd" in df_roteiros.columns:
        ...
        df_roteiros["cod_lograd"] = df_roteiros["cod_lograd"].str.replace(
            r"\D", "", regex=True
        )
        df_roteiros["cod_lograd"] = df_roteiros["cod_lograd"].str[
            :10
        ]  # Garante 10 dígitos
    ...
    df["NUM_ARGUMENTO3_COMPLEMENTO3"] = (
        df["COMPLEMENTO3_TRATADO"].str.extract(r"(\d+)")[0].fillna(0).astype(int)
    )
    ...
    colunas_para_remover = [
        "COMPLEMENTO3_ORIGINAL",
        "COMPLEMENTO3_TRATADO",
        "Resultado",
    ]
    ...
    print(
        f"   - Linhas sem prefixo válido: {(df['VALIDACAO'] == 'SEM PREFIXO VÁLIDO').sum():,}"
    )
...
def validar_colunas_csv(arquivo_path):
    ...
    try:
        ...
        df_header = pd.read_csv(
            arquivo_path, encoding="latin-1", sep=separador, nrows=0
        )
        colunas_encontradas = set(df_header.columns.str.strip().str.upper())
        colunas_obrigatorias_set = set(
            [coluna.upper() for coluna in COLUNAS_OBRIGATORIAS]
        )
...
def processar_csv_conversor(arquivo_path):
    ...
    try:
        ...
        if df_roteiro_aparecida is None or df_roteiro_goiania is None:
            raise Exception(
                "Erro ao carregar arquivos de roteiro. Verifique se os arquivos estão na pasta 'roteiros'."
            )

        # Processa os dados
        df_final = processar_enderecos_otimizado(
            df_enderecos, df_roteiro_aparecida, df_roteiro_goiania
        )
...
def processar_csv_conversor_grande(arquivo_path):
    ...
    try:
        ...
        update_progress(
            f"📊 Total de linhas encontradas: {total_rows:,}",
            progress=30,
            total=total_rows,
        )
        ...
        for chunk_number, chunk in enumerate(
            ...
            # Processa o chunk
            chunk_processado = processar_enderecos_otimizado(
                chunk, df_roteiro_aparecida, df_roteiro_goiania
            )
            ...
            update_progress(
                f"✅ Chunk {chunk_number} processado", progress=progress_percent
            )
