...
def criar_xml_edificio_ccomplementos(dados_csv, numero_pasta, complemento_vazio):
    ...
    codigo_zona = (
        str(dados_csv["COD_ZONA"])
        if "COD_ZONA" in dados_csv and not pd.isna(dados_csv["COD_ZONA"])
        else "DF-GURX-ETGR-CEOS-68"
    )
    ET.SubElement(edificio, "codigoZona").text = codigo_zona
    ET.SubElement(edificio, "nomeZona").text = codigo_zona
    localidade = (
        str(dados_csv["LOCALIDADE"])
        if "LOCALIDADE" in dados_csv and not pd.isna(dados_csv["LOCALIDADE"])
        else "GUARA"
    )
    ET.SubElement(edificio, "localidade").text = localidade
    endereco = ET.SubElement(edificio, "enderecoEdificio")
    ET.SubElement(endereco, "id").text = (
        str(dados_csv["ID_ENDERECO"])
        if "ID_ENDERECO" in dados_csv and not pd.isna(dados_csv["ID_ENDERECO"])
        else "93128133"
    )
    logradouro = str(
        dados_csv["LOGRADOURO"]
        + ", "
        + dados_csv["BAIRRO"]
        + ", "
        + dados_csv["MUNICIPIO"]
        + ", "
        + dados_csv["LOCALIDADE"]
        + " - "
        + dados_csv["UF"]
        + f" ({dados_csv['COD_LOGRADOURO']})"
    )
    ET.SubElement(endereco, "logradouro").text = logradouro
    num_fachada = (
        str(dados_csv["NUM_FACHADA"])
        if "NUM_FACHADA" in dados_csv and not pd.isna(dados_csv["NUM_FACHADA"])
        else "SN"
    )
    ...
    cep = (
        str(dados_csv["CEP"])
        if "CEP" in dados_csv and not pd.isna(dados_csv["CEP"])
        else "71065071"
    )
    ET.SubElement(endereco, "cep").text = cep
    bairro = (
        str(dados_csv["BAIRRO"])
        if "BAIRRO" in dados_csv and not pd.isna(dados_csv["BAIRRO"])
        else localidade
    )
    ET.SubElement(endereco, "bairro").text = bairro
    ET.SubElement(endereco, "id_roteiro").text = (
        str(dados_csv["ID_ROTEIRO"])
        if "ID_ROTEIRO" in dados_csv and not pd.isna(dados_csv["ID_ROTEIRO"])
        else "57149008"
    )
    ET.SubElement(endereco, "id_localidade").text = (
        str(dados_csv["ID_LOCALIDADE"])
        if "ID_LOCALIDADE" in dados_csv and not pd.isna(dados_csv["ID_LOCALIDADE"])
        else "1894644"
    )
    cod_lograd = (
        str(dados_csv["COD_LOGRADOURO"])
        if "COD_LOGRADOURO" in dados_csv and not pd.isna(dados_csv["COD_LOGRADOURO"])
        else "2700035341"
    )
    ...
    total_ucs = (
        int(dados_csv["QUANTIDADE_UMS"])
        if "QUANTIDADE_UMS" in dados_csv and not pd.isna(dados_csv["QUANTIDADE_UMS"])
        else 1
    )
...
def processar_csv(arquivo_path):
    ...
    estacao = (
        df["ESTACAO_ABASTECEDORA"].iloc[0]
        if "ESTACAO_ABASTECEDORA" in df.columns
        else "DESCONHECIDA"
    )
    diretorio_principal = (
        f'moradias_xml_{estacao}_{datetime.now().strftime("%Y%m%d%H%M%S")}'
    )
    ...
    for i, (index, linha) in enumerate(df.iterrows(), 1):
        ...
        coluna_complemento_2_vazia = (
            df["COMPLEMENTO3"].isna().all()
            or (df["COMPLEMENTO3"].astype(str).str.strip() == "").all()
        )
        ...
        xml_content = criar_xml_edificio_ccomplementos(
            linha, i, coluna_complemento_2_vazia
        )
        ...
        if i % 10 == 0 or i == 1:
            ...
            if coluna_complemento_2_vazia:
                log_processamento.append(
                    f'  COMP1("{comp1}" → código:{codigo1} argumento:"{arg1}")'
                )
                log_processamento.append(
                    f'  COMP2("{comp2}" → código:{codigo2} argumento:"{arg2}")'
                )
                ...
                log_processamento.append(
                    f'  COMP3("{resultado}" → código:{codigo3} argumento:"{arg3}")'
                )
                log_processamento.append("-" * 50)
